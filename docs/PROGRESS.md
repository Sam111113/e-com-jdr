...
- **Vérifié en conditions réelles :** aucune régression (84 tests unitaires, 27 E2E) ; le proxy vers `umami:3000` n'existe que dans le réseau Docker Compose du staging, donc pas testable depuis le conteneur de l'agent (isolé) — vérifié directement sur le staging après déploiement : une page visitée puis interrogée via l'API Umami (`/api/websites/.../pageviews`) montre bien un passage, et un envoi de test du formulaire de contact fait apparaître l'événement `contact_envoye`.
- **T1.14 terminée.**
